...
exports.findById = async (req, res) =>{
  try {
    ...
    if(!course){
      return res.status(404).send({status :`Curso de ID: ${id} não foi encontrado!`})
    }
...
exports.update = async (req, res) =>{
  try {
    ...
    if(!course){
      return res.status(404).send({ status: `Nenhum curso com o ID: ${ id } foi encontrado!`})
    };
...
exports.delete = async (req, res) =>{
  try {
    ...
    const course = await knex.select(['id']).where({ id }).first();

    if(!course){
      return res.status(404).send({status: `Curso de ID ${id} não encontrado`})
    }

    await knex.delete().from('courses').where({id : course.id})
